...
// 确保正面平面光存在
function ensureFrontalLight() {
    if (frontalLight) return;
    // 创建一个正面方向光作为平面光
    const light = new THREE.DirectionalLight(0xfff5e0, 0.3);
    light.position.set(0, 6, 22);
...
// 移除正面平面光
function removeFrontalLight() {
    if (frontalLight) {
        scene.remove(frontalLight);
        if (frontalLight.target) scene.remove(frontalLight.target);
        frontalLight = null;
    }
}
...
function initTopPointLights() {
    ...
    if (isDayMode) {
        ...
        gridParents.forEach(grid => {
            ...
            const light = new THREE.PointLight(0xffaa77, 0); // 白天强度为0
            light.distance = 6;
            light.decay = 1.2;
        ...
        console.log(`白天模式：为 ${topPointLights.length} 个格子创建了点光源（强度0）`);
    } else {
        // 黑夜模式：不创建点光源，确保有正面平面光
        ensureFrontalLight();
        console.log('黑夜模式：已移除所有点光源，并添加正面平面光');
    }
}
